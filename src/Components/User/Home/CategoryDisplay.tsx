  ...
  return (
    <div className='pl-2 pr-2 mb-10'>
      <div className="w-full md:h-full lg:h-full sm:h-full bg-slate-200 p-2  hover:scale-x-100 rounded-xl overflow-hidden">
        ...
        <div className="w-full sm:h-fit md:h-fit lg:h-fit mt-2 sm:grid sm:grid-cols-2 sm:gap-2  md:grid md:grid-cols-5 gap-3">
        {
          categories?.map((items)=>(
            items.Status?
          <div key={items._id} className="sm:w-full sm:h-20 md:h-20 md:w-full relative overflow-hidden bg-transparent group hover:bg-black hover:shadow-2xl hover:-translate-y-1 hover:translate-x-1 hover:cursor-pointer  rounded-xl " onClick={()=>{navigate(`SubCategory/${items.Category}`)}}>
